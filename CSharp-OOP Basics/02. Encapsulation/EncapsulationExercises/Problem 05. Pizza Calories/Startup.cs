...
namespace Problem_05.Pizza_Calories
{
	class Startup
	{
		...
		private static void PrintPizza(string input)
		{
			...
			var doughWeight = decimal.Parse(doughData[3]);
			...
			for (int i = 0; i < toppingsCount; i++)
			{
				...
				var toppingWeight = decimal.Parse(toppingTokens[2]);
		...
		private static void PrintTopping(string input)
		{
			...
			var test2Topping = new Topping(toppingTokens[1], decimal.Parse(toppingTokens[2]));
		...
		private static void PrintDough(string input)
		{
			...
			var test1Dough = new Dough(test1tokens[1], test1tokens[2], decimal.Parse(test1tokens[3]));
